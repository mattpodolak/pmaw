...
# TODO: fix this test - JSONDecodeError
@tape.use_cassette()
...
# TODO: run when reddit credentials are available
# @tape.use_cassette()
# def test_submission_praw_mem_safe():
#   api_praw = PushshiftAPI(file_checkpoint=1, praw=reddit)
#   posts = api_praw.search_submissions(subreddit="programming", limit=1000, mem_safe=True, until=1671827157)
#   assert(len(posts) == 1000)
...
#   api_praw = PushshiftAPI(file_checkpoint=1, praw=reddit)
#   posts = api_praw.search_submissions(q="ai", subreddit="programming", limit=100, until=1671827157)
#   assert(len(posts) == 100)
