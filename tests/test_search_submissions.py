...
def test_submission_search_limit():
  ...
  posts = api.search_submissions(subreddit="science", limit=100, since=1671827157)
  assert(len(posts) == 100)

# @tape.use_cassette()
# def test_submission_praw_limit():
#   api_praw = PushshiftAPI(file_checkpoint=1, praw=reddit)
#   posts = api_praw.search_submissions(subreddit="programming", limit=100, since=1671827157)
#   assert(len(posts) == 100)

# @tape.use_cassette()
# def test_submission_search_query():
#   api = PushshiftAPI(file_checkpoint=1)
#   posts = api.search_submissions(q="ai", subreddit="programming", limit=100, since=1671827157)
#   assert(len(posts) == 100)

# @tape.use_cassette()
# def test_submission_praw_query():
#   api_praw = PushshiftAPI(file_checkpoint=1, praw=reddit)
#   posts = api_praw.search_submissions(q="ai", subreddit="programming", limit=100, since=1671827157)
#   assert(len(posts) == 100)
...
#   api = PushshiftAPI(file_checkpoint=1)
#   posts = api.search_submissions(subreddit="programming", limit=1000, mem_safe=True, since=1671827157)
#   assert(len(posts) == 1000)

# @tape.use_cassette()
# def test_submission_praw_mem_safe():
#   api_praw = PushshiftAPI(file_checkpoint=1, praw=reddit)
#   posts = api_praw.search_submissions(subreddit="programming", limit=1000, mem_safe=True, since=1671827157)
#   assert(len(posts) == 1000)